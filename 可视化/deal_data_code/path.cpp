...
int main()
{
    ...
    for (int pp = 0; pp < path.size(); pp++)
    {
        ...
        map<string, int> temp;
        string a="台湾省",b="北京市",c="上海市",d="天津市",e="重庆市";
        for (int i = 0; i < data_line.size(); i++)
        {
            if (data_line[i].geo[0] == "[]" || data_line[i].geo[0] == "中华人民共和国" || 
            ((strcmp(data_line[i].geo[0].c_str(),a.c_str())!=0&&data_line[i].geo[1] == "[]")&&
            (strcmp(data_line[i].geo[0].c_str(),b.c_str())!=0&&data_line[i].geo[1] == "[]")&&
            (strcmp(data_line[i].geo[0].c_str(),c.c_str())!=0&&data_line[i].geo[1] == "[]")&&
            (strcmp(data_line[i].geo[0].c_str(),d.c_str())!=0&&data_line[i].geo[1] == "[]")&&
            (strcmp(data_line[i].geo[0].c_str(),e.c_str())!=0&&data_line[i].geo[1] == "[]")))
            {
                continue;
            }
            else
            {
                if (duiying.count(data_line[i].geo[0] + data_line[i].geo[1]) == 0)
                {
                    temp.insert(make_pair(data_line[i].geo[0] + data_line[i].geo[1], 1));
                    duiying.insert(make_pair(data_line[i].geo[0] + data_line[i].geo[1], j));
                    j++;
                    result.emplace_back(data_line[i]);
                }
                else
                {
                    auto it = duiying.find(data_line[i].geo[0] + data_line[i].geo[1]);
                    auto tit = temp.find(data_line[i].geo[0] + data_line[i].geo[1]);
                    tit->second += 1;
                    int index = it->second;
                    for (int t = 0; t < 6; t++)
                    {
        ...
        {
            string city = result[i].geo[0] + result[i].geo[1];
        ...
        ofstream outFile;
        string pdir = "D:\\DA-code\\deal-city-data\\" + p2[pp] + "\\";
        if (_access(pdir.c_str(), 0) == -1)
        {
            _mkdir(pdir.c_str());
        }
        string ppp = "D:\\DA-code\\deal-city-data\\" + p2[pp] + "\\" + name[pp];
        outFile.open(ppp);
        outFile << "city,AQI,质量等级" << endl;
        for (int i = 0; i < result.size(); i++)
        {
            string rs = "";
            rs += result[i].geo[0];
